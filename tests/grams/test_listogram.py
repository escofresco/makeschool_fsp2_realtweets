...
class ListogramTest(unittest.TestCase):
    ...
    def test_add_count(self):
        ...
        assert histogram.type_count == 6
        # Verify total count of all word tokens
        assert histogram.token_count == 8 + 14

    def test_tokens(self):
        histogram = Listogram(self.fish_words)
        # Verify total count of all word tokens
        assert len(self.fish_words) == 8
        assert histogram.token_count == 8
        ...
        assert histogram.token_count == 8 * 2

    def test_types(self):
        histogram = Listogram(self.fish_words)
        # Verify count of distinct word types
        assert len(set(self.fish_words)) == 5
        assert histogram.type_count == 5
    ...
    def test_sample(self):
        ...
        for word, count in histogram.bins:
            # Calculate word's observed frequency
            observed_freq = count / histogram.token_count
            # Calculate word's sampled frequency
            samples = samples_hist.frequency(word)
            sampled_freq = samples / samples_hist.token_count

...
@dataclass
class TD:
    COLOR_TUPLE_FREQS = (("orange", 10), ("yellow", 16), ("green", 8), ("blue", 20),
                   ("grey", 8), ("indigo", 8), ("pink", 10))
    COLOR_TUPLE_PROBS = (("orange", Fraction(1, 8)), ("yellow", Fraction(1, 5)), ("green", Fraction(1, 10)), ("blue", Fraction(1, 4)),
                   ("grey", Fraction(1, 10)), ("indigo", Fraction(1, 10)), ("pink", Fraction(1, 8)))
...
class SampleTestSuite(unittest.TestCase):
    ...
    def test_preprocessing(self):
        N_SAMPLES = 10000
        freqdist = FreqDist(TD.COLOR_TUPLE_FREQS)
        ...
        expected_outcomes, expected_probs = zip(*freqdist.padded_bins(freqdist.max_token_str_len))
        freqdist.show_data(expected_outcomes, map(lambda prob: float(prob)*100, expected_probs))
        ...
        actual_outcomes, actual_probs = zip(*samples.padded_bins(samples.max_token_str_len))
        samples.show_data(actual_outcomes, map(lambda prob: float(prob)*100, actual_probs))

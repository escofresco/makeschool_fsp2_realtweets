...
a modified version of the termgraph module in the termgraph PyPI termgraph
library
...
def showgraph(**args):
    """takes at minimum data and corresponding labels.

    ex:     labels: ['2007', '2008', '2009', '2010', '2011', '2014']
            data: [[183.32], [231.23], [16.43], [50.21], [508.97], [1.0]]"""
    ...
    if not (len(args["labels"]) and len(args["data"])):
        raise ValueError("labels and data can't be empy.")

    if any(not isinstance(label, str) for label in args["labels"]):
...
def chart(colors, data, args, labels):
    ...
    if len_categories > 1:
        ...
        if args["stacked"]:
            ...
            stacked_graph(labels, data, normal_dat, len_categories, args,
                          colors)
            return
        ...
        if args["different_scale"]:
            for i in range(len_categories):
                ...
                for row in horiz_rows(labels, cat_data, normal_cat_data, args,
                                      [colors[i]]):
                    # Print the row
                    if not args["vertical"]:
                        print_row(*row)
...
def calendar_heatmap(data, labels, args):
    ...
    if args["start_dt"]:
        ...
        start = datetime.now()
        start_dt = datetime(year=start.year - 1,
                            month=start.month,
                            day=start.day)
